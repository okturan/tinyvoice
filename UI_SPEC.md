...
**One-hour history:** the relay stores each room's packets and replays them to anyone who joins. Replayed messages appear in the list — with their original send time, and attributed to you if you sent them — but must never auto-play, and are separated from live ones by a "replayed from the last hour" divider. Because a reconnect replays the backlog into a list that was never cleared, replayed messages are deduped against what is already shown. The list caps at 1,000 messages and clears when switching rooms (not on a transient reconnect). The room list states the retention plainly, and the empty state reads "Voice messages from the last hour appear here".
